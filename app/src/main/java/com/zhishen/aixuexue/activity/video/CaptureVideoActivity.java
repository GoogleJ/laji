...
//                  recordingState.setBackgroundResource(R.drawable.nim_record_start);
    ...
    private void parseIntent() {
        ...
        isMoment = getIntent().getExtras().getBoolean(Constant.SMALL_FROM_MOMENT, false);
    }
    ...
    private void setViewsListener() {
        recordBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (recording) {
                    if (!CommonUtils.isFastDoubleClick(R.id.record_btn)) {
                        stopRecorder();
                        sendVideo();
                    }
                } else {
        ...
        if (super.isCompatible(11)) {
            ...
            if (super.isCompatible(15)) {
                if (CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_HIGH_SPEED_720P)) {
                    profile = CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_HIGH_SPEED_720P);
                    if (profile != null) {
                        Point point = new Point();
                        point.x = profile.videoFrameWidth;
                        point.y = profile.videoFrameHeight;
    ...
    public void updateRecordUI() {
        if (recording) {
            recordBtn.setImageResource(R.drawable.video_recorder_start_btn);
        } else {
            recordBtn.setImageResource(R.drawable.video_recorder_stop_btn);
        }
    }
    ...
    private void startRecorder() {
        try {
            ...
            CommonUtils.showToastShort(this, R.string.record_failed);
