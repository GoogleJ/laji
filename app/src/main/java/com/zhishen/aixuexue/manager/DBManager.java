...
public class DBManager {
    ...
    private DBManager() {
        dbHelper = DbOpenHelper.getInstance(AiApp.getInstance().getApplicationContext());
    }
    ...
    synchronized public List<InviteMessage> getMessagesList() {
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (status == InviteMessage.Status.BEINVITEED.ordinal())
                    msg.setStatus(InviteMessage.Status.BEINVITEED);
                else if (status == InviteMessage.Status.BEAGREED.ordinal())
                    msg.setStatus(InviteMessage.Status.BEAGREED);
                else if (status == InviteMessage.Status.BEREFUSED.ordinal())
                    msg.setStatus(InviteMessage.Status.BEREFUSED);
                else if (status == InviteMessage.Status.AGREED.ordinal())
                    msg.setStatus(InviteMessage.Status.AGREED);
                else if (status == InviteMessage.Status.REFUSED.ordinal())
